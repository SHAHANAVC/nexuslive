  ...
  const exportToExcel = () => {
    try {
      const worksheet = XLSX.utils.json_to_sheet(payments.map(payment => ({
        ...
        'Name': payment.memberName || 'N/A',
        'Category': payment.category,
        'Form ID': payment.formId || payment.groupNo || 'N/A',
  ...
  const exportToPDF = () => {
    try {
      if (!payments || payments.length === 0) {
        setError('No payments data to export');
        return;
      }
      ...
      if (filters.startDate || filters.endDate) {
        filterInfo.push(`Date Range: ${filters.startDate || 'Start'} to ${filters.endDate || 'End'}`);
      }
      if (filters.category !== 'all') {
        filterInfo.push(`Category: ${filters.category}`);
      }
      if (filters.paymentMethod !== 'all') {
        filterInfo.push(`Method: ${filters.paymentMethod}`);
      }
      
      const infoText = filterInfo.length > 0 ? filterInfo.join(' | ') : 'All Payments';
      doc.text(infoText, 105, 28, { align: 'center' });

      // Summary
      doc.setFontSize(9);
      doc.setTextColor(0, 0, 0);
      const totalAmount = payments.reduce((sum, payment) => sum + payment.paidAmount, 0);
      doc.text(`Total Payments: ${payments.length} | Total Amount: ${formatAmount(totalAmount)}`, 14, 38);
      ...
      // ✅ FIXED: Use autoTable as a separate function, not as doc.autoTable
      autoTable(doc, {
        startY: 45,
        head: [
          ['#', 'Date', 'Name', 'Category', 'Form ID', 'Amount', 'Method', 'Institution']
        ],
        body: tableData,
        theme: 'grid',
        headStyles: {
      ...
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text(`Generated on ${new Date().toLocaleString('en-IN')}`, 14, finalY);
